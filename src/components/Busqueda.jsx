...
const Busqueda = () => {
    ...
    // afterLeave - es un hook que indica que pasa despues que se cierra, dejamos state vacio
    return (
                        ...
                        <Combobox.Options static className="max-h-72 scroll-py-2 overflow-y-auto py-2 text-sm text-gray-800">
                            {proyectosFiltrados.map(proyecto =>
                                <Combobox.Option
                                    key={proyecto._id}
                                    proyecto={proyecto}
                                    className={({active}) => classNames('mb-1 cursor-default select-none px-4 py-2', active && 'bg-sky-600 text-white rounded-sm')}
                                    >
                                        {proyecto.nombre}
                                </Combobox.Option>
                            )}
